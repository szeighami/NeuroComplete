...
class Phi():
    def __init__(self, out_dim, in_dim, init_width, mid_width, no_layers, name='base'):
        ...
        for i in range(no_layers):
            output = tf.keras.layers.Dense(mid_width, activation=tf.nn.relu, kernel_initializer='he_uniform')(output)
            #output = tf.keras.layers.Dense(mid_width, activation=tf.nn.relu, kernel_initializer='he_uniform', kernel_regularizer=tf.keras.regularizers.L2(l2=0.001))(output)
